...
function validateHikeData(data) {
  ...
  if (
    data.difficulty &&
    !Object.values(difficultyLevels).includes(data.difficulty)
  ) {
    errors.push(
      `difficulty must be one of: ${Object.values(difficultyLevels).join(', ')}`
    );
  }

  if (data.status && !Object.values(hikeStatus).includes(data.status)) {
    errors.push(
      `status must be one of: ${Object.values(hikeStatus).join(', ')}`
    );
  }
