    ...
    let suggestions = snapshot.docs
      ...
      .map((doc) => {
        ...
        return {
          ...
          mutualFriends: (data.friends || []).filter((f) =>
            (currentUserData.friends || []).includes(f)
          ).length,
  ...
  try {
    ...
    for (const doc of snapshot.docs) {
      ...
      results.push({
        id: doc.id,
        from: data.from,
        fromName: userData.displayName || userData.name || '',
  ...
  try {
    ...
    if (action === 'accept') {
      ...
      await db
        .collection('users')
        .doc(data.from)
        .update({ friends: FieldValue.arrayUnion(data.to) });
      await db
        .collection('users')
        .doc(data.to)
        .update({ friends: FieldValue.arrayUnion(data.from) });
      await db
