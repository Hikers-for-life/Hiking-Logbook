...
export const dbUtils = {
  ...
  async getUserHikes(userId, filters = {}) {
    try {
      console.log(`getUserHikes: Getting hikes for user ${userId} with filters:`, filters);
      const db = getDatabase();
      let query = db
        .collection('users')
        .doc(userId)
      ...
      console.log(`getUserHikes: Query snapshot size: ${snapshot.size} for user ${userId}`);
      
      let hikes = [];
      snapshot.forEach(doc => {
        const hikeData = { id: doc.id, ...doc.data() };
        console.log(`getUserHikes: Found hike:`, { id: doc.id, status: hikeData.status, title: hikeData.title });
        hikes.push(hikeData);
      });
  ...
  async getUserHikeStats(userId) {
    try {
      ...
      console.error(`Failed to get hike stats for user ${userId}:`, error);
