...
const Logbook = () => {
  ...
  const loadHikes = useCallback(
    async (searchTerm = '', difficultyFilter = 'All') => {
      if (!user) {
        return;
      }
  ...
  const handleCompleteActiveHike = async (hikeData) => {
    try {
      ...
      const response = await hikeApiService.completeHike(
        currentActiveHike.id || currentActiveHike.activeHikeId,
        endData
      );
      ...
      if (response.success) {
        if (currentActiveHike.plannedHikeId) {
          try {
            ...
            await plannedHikeApiService.deletePlannedHike(
              currentActiveHike.plannedHikeId
            );
  ...
  const handleStartActiveHike = async (formData) => {
    try {
      const hikeData = {
        ...
        status: 'active',
        ...(formData?.plannedHikeId && {
          plannedHikeId: formData.plannedHikeId,
        }),
      };
  ...
  const handleSubmitEditHike = async (updatedHikeData) => {
    ...
    try {
      const response = await hikeApiService.updateHike(
        editingHike.id,
        updatedHikeData
      );
  ...
  const handlePinHike = async (hikeId) => {
    try {
      ...
      if (isPinned) {
        ...
        setHikeEntries((prev) =>
          prev.map((h) => (h.id === hikeId ? { ...h, pinned: false } : h))
        );
      } else {
        await hikeApiService.pinHike(hikeId);
        setHikeEntries((prev) =>
          prev.map((h) => (h.id === hikeId ? { ...h, pinned: true } : h))
        );
      }
    } catch (error) {
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-muted-foreground text-lg">
                Keep notes on location, weather, elevation, and route - along
                the way
              </p>
            </div>
          ...
          <div className="space-y-6">
            ...
            {!isLoading && !error && filteredHikes.length === 0 ? (
              <Card className="bg-card border-border text-center py-12">
                <CardContent>
                  <p className="text-muted-foreground text-lg">
                    No hikes found matching your search criteria.
                  </p>
              ...
              filteredHikes.map((hike, index) => (
                ...
                >
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {hike.accomplishments &&
                      hike.accomplishments.length > 0 && (
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            {hike.accomplishments.map(
                              (accomplishment, index) => (
                                <div
                                  key={accomplishment.id || index}
                                  className="flex items-start gap-3 p-3 bg-gradient-to-r from-forest/5 to-meadow/5 rounded-lg border border-forest/20"
                                >
