...
jest.mock('../contexts/AuthContext.jsx', () => {
  ...
  return {
    useAuth: () => ({ currentUser: { uid: 'u1', displayName: 'Tester' }, signup: jest.fn(), signInWithGoogle: jest.fn() }),
...
jest.mock('../services/hikeApiService.js', () => ({
  hikeApiService: {
    getHikes: jest.fn().mockResolvedValue({ success: true, data: [{ id: 'h1', title: 'Trail One', date: '2024-01-01', distance: '3', difficulty: 'Easy' }] }),
...
jest.mock('../services/userService', () => ({
  userApiService: {
    getCurrentProfile: jest.fn().mockResolvedValue({ displayName: 'Hiker', bio: '', location: '' }),
    updateProfile: jest.fn().mockResolvedValue({ profile: { displayName: 'New', bio: '', location: 'Cape Town' } })
  },
  locationService: {
    getLocationCoordinates: jest.fn().mockResolvedValue({ latitude: -33.9, longitude: 18.4 })
  }
}));

jest.mock('../services/userServices', () => ({ searchUsers: jest.fn(async (q) => (q ? [{ id: 'x', displayName: 'John Walker', location: 'CT' }] : [])) }));
jest.mock('../services/statistics', () => ({ getUserStats: jest.fn().mockResolvedValue({}) }));
jest.mock('../services/friendService.js', () => ({ getFriendProfile: jest.fn().mockResolvedValue({ success: true, recentHikes: [] }) }));
jest.mock('../services/discover', () => ({ discoverFriends: jest.fn().mockResolvedValue([]), addFriend: jest.fn().mockResolvedValue({}) }));
jest.mock('../services/feed', () => ({ fetchFeed: jest.fn().mockResolvedValue([]), likeFeed: jest.fn(), commentFeed: jest.fn(async (_id, content) => ({ comment: { id: 'c1', name: 'T', userId: 'u1', content } })), shareFeed: jest.fn(async () => ({ id: 's1' })), fetchComments: jest.fn(), deleteCommentFeed: jest.fn(), deleteFeed: jest.fn() }));
...
describe('Page interactions', () => {
  ...
  test.skip('Friends allows typing in search and displays result card', async () => {
    ...
    const input = await screen.findByPlaceholderText(/Search for friends or hikers/i);
  ...
  test.skip('Signup renders and allows typing placeholders', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Enter your full name/i), { target: { value: 'User A' } });
    fireEvent.change(screen.getByPlaceholderText(/your@gmail.com/i), { target: { value: 'a@a.com' } });
