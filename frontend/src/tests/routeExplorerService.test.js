...
describe('routeExplorerService', () => {
  ...
  beforeAll(() => {
    routeExplorerService = require('../services/routeExplorerService').routeExplorerService;
  });
  ...
  test('discoverNearbyTrails builds POST payload and parses features', async () => {
    const mockFeatures = [
      {
        properties: { osm_id: 1, osm_tags: { name: 'Trail A', surface: 'rock' } },
  ...
  test('discoverNearbyTrails throws on bad response', async () => {
    ...
    await expect(routeExplorerService.discoverNearbyTrails(-33.9, 18.4, 5)).rejects.toThrow('Failed to fetch data');
  });
});
