...
const ActiveHike = ({ hikeId, onComplete, onSave, initialData }) => {
        ...
        const id = navigator.geolocation.watchPosition(
          (position) => {
            ...
            if (previousLocation && isActive) {
              ...
              if (distanceKm > 0.005 && accuracy < 50) { // 5 meters threshold, 50m accuracy
                ...
                // Auto-save waypoint every significant movement
                setHikeData(prev => {
                  const newWaypoint = {
                    id: Date.now(),
                    ...newLocation,
  ...
  const handleStopHike = () => {
    ...
    // Add final waypoint if we have current location
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Waypoints List - Only show manually added waypoints */}
        {hikeData.waypoints.filter(wp => !wp.autoGenerated).length > 0 && (
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>GPS Waypoints Marked</CardTitle>
                <Badge variant="outline" className="text-xs">
                  {hikeData.waypoints.filter(wp => !wp.autoGenerated).length} waypoints
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {hikeData.waypoints.filter(wp => !wp.autoGenerated).map((waypoint, index) => (
